...
@section('content')
    <form enctype="multipart/form-data" method="POST" onsubmit="event.preventDefault(); createproduct();">
        <div class="form-group col-md-6">
            ...
            <input type="text" class="form-control" value="{{$product->name}}" id="name" placeholder="Enter product's Name">
        </div>
        <div class="form-group col-md-6">
            <label for="barcode">Barcode</label>
            <input type="text" class="form-control" value="{{$product->barcode}}" id="barcode" placeholder="Enter product's Barcode">
        </div>
        <div class="form-group col-md-6">
        ...
        <div class="form-group col-md-6">
            ...
            <textarea id="description" cols="30" rows="10" class="form-control" placeholder="Enter product's Description">{{$product->description}}</textarea>
        </div>
        ...
        <div class="form-group col-md-6">
            ...
            <br>
            <input type="text" data-role="tagsinput" value="{{$product->keywords_as_string}}" class="form-control" id="keywords"
        ...
        <div class="row">
            <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control" id="price" value="{{$product->price}}" placeholder="Enter product's Price">
            </div>
            <div class="form-group col-md-3">
                ...
                <select id="coin" class="form-select" aria-label="Default select example">
                    ...
                    @foreach ($coins as $coin)
                        <option value="{{$coin->id}}" @selected($coin->id == $product->coin_id)>{{$coin->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div style="margin-bottom: 20px" class="form-check form-switch">
            <input class="form-check-input" type="checkbox" id="active" @checked($product->is_active)>
...
@section('script')
    ...
    <script>
        function createproduct() {
            ...
            post("{{ route('products.update', $product) }}", formData, 'button-update', "{{ route('products.index') }}");
        }
    </script>
